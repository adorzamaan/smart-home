...
const orderHandler = ()=>{
  if(cart.length){
    setCart([]);
  deleteShoppingCart();
  return toast.success('Order Placed success',{autoClose:500})
  }
    ...
    <div className='flex min-h-screen items-start justify-center bg-gray-100 text-gray-900'>
      <div className='flex flex-col max-w-3xl p-6 space-y-4 sm:p-10 '>
        ...
        <ul className='flex flex-col divide-y divide-gray-700'>
          {
            cart.map(product => <CartItem key={product.id}  product={product} handleRomoveItem={handleRomoveItem}></CartItem>)
          }
        </ul>
